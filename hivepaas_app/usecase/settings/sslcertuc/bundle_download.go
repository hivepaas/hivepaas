...
func (uc *UC) DownloadBundle(
	...
	if err != nil {
		return nil, apperrors.Wrap(err).WithMsgLog("failed to create zip entry for certificate")
	}
	if _, err := certWriter.Write([]byte(sslCert.Certificate)); err != nil {
		return nil, apperrors.Wrap(err).WithMsgLog("failed to write certificate to zip")
	}

	// 2. Add private.key
	keyWriter, err := zipWriter.Create(keyEntryName)
	if err != nil {
		return nil, apperrors.Wrap(err).WithMsgLog("failed to create zip entry for private key")
	}
	if _, err := keyWriter.Write([]byte(privateKey)); err != nil {
		return nil, apperrors.Wrap(err).WithMsgLog("failed to write private key to zip")
	}
	...
	if strings.TrimSpace(sslCert.CACertificate) != "" {
		...
		if err != nil {
			return nil, apperrors.Wrap(err).WithMsgLog("failed to create zip entry for CA certificate")
		}
		if _, err := caWriter.Write([]byte(sslCert.CACertificate)); err != nil {
			return nil, apperrors.Wrap(err).WithMsgLog("failed to write CA certificate to zip")
		}
	}

	if err := zipWriter.Close(); err != nil {
		return nil, apperrors.Wrap(err).WithMsgLog("failed to finalize zip archive")
	}
