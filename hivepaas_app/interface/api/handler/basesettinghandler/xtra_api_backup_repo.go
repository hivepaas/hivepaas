...
func (h *Handler) BackupRepoSync(ctx *gin.Context, scopeType base.ObjectScopeType) {
	var auth *basedto.Auth
	var itemID string
	var err error
	...
	req := backuprepodto.NewSyncBackupRepoReq()
	req.Scope = scope
	req.Type = base.SettingTypeBackupRepo
	req.ID = itemID
	if err = h.ParseAndValidateJSONBody(ctx, req); err != nil {
	...
	resp, err := h.BackupRepoUC.SyncBackupRepo(h.RequestCtx(ctx), auth, req)
	if err != nil {
		h.RenderError(ctx, err)
		return
	}
