...
func (s *HTTPServer) registerProjectRoutes(apiGroup *gin.RouterGroup) {
	...
	{ // Backup repository group
		...
		backupRepoGroup.POST("", projectSettingsHandler.CreateBackupRepo)
	...
	{ // Env vars
		...
		envVarGroup.POST("/compute", projectSettingsHandler.BuildEnvVars)
	}
	...
	{ // Task group
		...
		taskGroup.GET("/types", projectHandler.ListTaskType)
