...
// makeKey derives an AES key from a secret and a salt.
//
// This is deliberately slow and memory-hard, because the secret it derives from
// is chosen by whoever deploys the app and may be weak. It is only used to wrap
// and unwrap the data encryption key, once per process at startup - the values
// themselves are sealed with that key directly, with no derivation at all.
