...
func TestRuntimeSpecCarriesRetentionAndStorage(t *testing.T) {
	c := New(&Config{DataVolume: "vol-123", Retention: 14 * 24 * time.Hour})
	...
	assert.Equal(t, "vol-123", spec.Mounts[0].Volume)
...
func TestRuntimeSpecRoundsRetentionUpToWholeDays(t *testing.T) {
	c := New(&Config{DataVolume: "v", Retention: 30 * time.Minute})
...
func TestRuntimeSpecOmitsDiskLimitWhenUnset(t *testing.T) {
	c := New(&Config{DataVolume: "v", Retention: time.Hour})
...
func TestRuntimeSpecPassesDiskLimit(t *testing.T) {
	c := New(&Config{DataVolume: "v", Retention: time.Hour, MaxDiskUsagePercent: 80})
